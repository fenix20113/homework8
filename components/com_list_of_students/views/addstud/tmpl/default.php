...
<script type="text/javascript">
    window.addEvent('domready', function(){
        document.formvalidator.setHandler('group', function(value) {
            regex=/^\b\d{1}[А-ЯA-Z]{1}-\d{2}\b$/;
            return regex.test(value);
        });
    });
...
<script type="text/javascript">
    window.addEvent('domready', function(){
        document.formvalidator.setHandler('user', function(value) {
            regex=/^\b[А-ЯA-Z]{1}[a-zа-я]{1,15}\b$/;
            return regex.test(value);
        });
    });
</script>
...
<style>
.front-end-edit ul li p {float: right;}
    ...
    <form id="form-ofstudents" action="<?php echo JRoute::_('index.php?option=com_list_of_students&task=ofstudents.save'); ?>" method="post" class="form-validate" enctype="multipart/form-data">
        <ul>
            ...
            <li><?php echo $this->form->getLabel('surename'); ?>
                <?php echo $this->form->getInput('surename'); ?><p>Введіть прізвище з великої літери</p></li>
            <li><?php echo $this->form->getLabel('firstname'); ?>
                <?php echo $this->form->getInput('firstname'); ?><p>Введіть ім’я з великої літери</p></li>
            <li><?php echo $this->form->getLabel('lastname'); ?>
                <?php echo $this->form->getInput('lastname'); ?><p>Введіть ім’я по-батькові з великої літери</p></li>
            <li><?php echo $this->form->getLabel('info'); ?>
                <?php echo $this->form->getInput('info'); ?></li>
            <li class="year"><?php echo $this->form->getLabel('year');?>
                <?php echo $this->form->getInput('year'); ?><p>Введіть дату народження в форматі yyyy-mm-dd</p></li>
            <li class="photo"><?php echo $this->form->getLabel('photo'); ?>
                <?php echo $this->form->getInput('photo'); ?></li>
            <li><?php echo $this->form->getLabel('group'); ?>
                <?php echo $this->form->getInput('group'); ?><p>Введіть назву групи в форматі xy-xx, де х-цифра,у-велика літера. Наприклад, 2К-09</p></li>
            <li><?php echo $this->form->getLabel('phone'); ?>
                <?php echo $this->form->getInput('phone'); ?><p>Введіть номер телефону в форматі 80ххххххххх</p></li>
